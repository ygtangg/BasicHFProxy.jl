...
function bhfp_mpi(inputfile = get_input_filename_from_args(); verbose = false)
    ...
    if myrank != 0
        xpnt = JACC.Array{Float64, 1}(undef, ngauss)
        coef = JACC.Array{Float64, 1}(undef, ngauss)
        geom = JACC.Array{Float64, 2}(undef, 3, natom)
    end
    ...
    dens = JACC.Array{Float64, 2}(undef, natom, natom)

    # fake ('random') density
    function _init_fields_kernel!(ly, lz, u, v, offsets, sizes, minL, maxL)

    function fill_dens_kernel!(i, j, dens)
         i == j ? dens[i, j] = 1.0 : dens[i, j] = 0.1
    end

    JACC.parallel_for((natom,natom), fill_dens_kernel!, dens)
    # the above functional form replaces the two for loops below
    # for i in 1:natom
    #     for j in 1:natom
    #         dens[i, j] = 0.1
    #     end
    ...
    fock = JACC.zeros(Float64, natom, natom)
    frcv = JACC.Array{Float64,2}(undef, natom, natom)

    # normalize the primitive GTO weights
    function normalize_coef_kernel!(i, coef, xpnt)
        coef[i] = coef[i] * (2.0 * xpnt[i])^0.75
    end
    JACC.parallel_for(ngauss, normalize_coef_kernel!, coef, xpnt)

    # the above functional form replaces the one for loop below
    # for i in 1:ngauss
    #     coef[i] = coef[i] * (2.0 * xpnt[i])^0.75
    # end

    # scale the geometry to Bohrs for energy calculations in AU
    function scale_geom_kernel!(i, geom)
        geom[1, i] = geom[1, i] * tobohrs
        geom[2, i] = geom[2, i] * tobohrs
        geom[3, i] = geom[3, i] * tobohrs
    end
    JACC.parallel_for(natom, scale_geom_kernel!, geom)
